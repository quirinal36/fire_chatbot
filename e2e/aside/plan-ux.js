 ...
 * 6. 단계 칩이 곧 도구 카드다. 다른 칩을 눌러 두면 '지금 할 일' 이 안내로 바뀌고, 캔버스와 바닥 띠는 화면 안에 있다
 *
...
// 도구는 그 단계의 카드에 있다. 카드를 먼저 연다
const goStep = async (n) => { await page.locator(n === 0 ? '.plan3d__todo-back' : '.plan3d__step[data-step="' + n + '"]').click(); await sleep(200); };

await openTab(base);
await page.waitForSelector('.app[data-ready="true"]', { timeout: 20000 });
// 패널 폭에 따라 배치가 달라진다. 다른 시험이 남긴 폭에 흔들리지 않게 기본 폭으로 고정한다
...
// 6. 단계 칩 = 도구 카드. 지금 단계가 아닌 칩을 누르면 안내로 바뀌고, 돌아가기로 원래 문구가 된다
const todoText = () => page.evaluate(() => document.querySelector('.plan3d__todo-label').textContent.trim());
await page.locator('[data-action="mode"][data-mode="view"]').click();
await sleep(200);
await goStep(4);
const pinnedTodo = await todoText();
const card4 = await page.evaluate(() => !document.querySelector('.plan3d__card[data-step="4"]').hidden);
check('다른 칩을 누르면 그 카드가 열리고 지금 할 일을 안내한다', card4 && pinnedTodo.indexOf('지금 할 일은') >= 0, pinnedTodo);
await goStep(0);
const backTodo = await todoText();
check('돌아가기를 누르면 지금 할 일 문구로 돌아온다', backTodo.indexOf('지금 할 일 ·') === 0, backTodo);
const geo = await page.evaluate(() => {
  ...
  return { share: st.height / body.height, totalBottom: total.bottom, win: window.innerHeight, stage: Math.round(st.height) };
});
check('캔버스가 패널 본문의 60% 이상', geo.share >= 0.6, Math.round(geo.share * 100) + '% (' + geo.stage + 'px)');
check('바닥 면적이 화면 안에 있다', geo.totalBottom <= geo.win, Math.round(geo.totalBottom) + ' <= ' + geo.win);
await page.locator('[data-action="drawer"][data-drawer="rooms"]').click();
await sleep(300);
const roomsShown = await page.evaluate(() => !document.querySelector('.plan3d__drawer').hidden && document.querySelectorAll('.plan3d__room').length > 0);
await page.keyboard.press('Escape');
await sleep(200);
const roomsClosed = await page.evaluate(() => document.querySelector('.plan3d__drawer').hidden);
check('구역 목록은 서랍으로 열리고 Esc 로 닫힌다', roomsShown && roomsClosed, String(roomsShown) + ' -> ' + String(roomsClosed));
mark('단계 칩·서랍 확인');
