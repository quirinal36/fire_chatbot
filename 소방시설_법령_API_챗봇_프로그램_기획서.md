...
구현이 앞서 나간 두 가지는 기능 범위 결정이 필요하다. 이 문서의 범위를 임의로 넓히지 않고 여기에만 적어 둔다.

- 우측 패널의 `도면` 탭과 평면도 표시는 §2.2에서 후속 범위로 둔 도면 분석에 해당한다. 현재 고정 데이터이며 1차 출시에서 감추거나, 범위를 넓히는 결정을 따로 받아야 한다. 도면 첨부 버튼은 동작하지 않는다.
- 로그인 모달이 Google·카카오 진입점을 제공한다. §3 구성표의 인증 항목은 `서명된 익명 세션`만 규정한다. 소셜 로그인 채택 여부는 미결이다.

비로그인 사용자도 질문할 수 있고, 비회원 대화는 저장하지 않는다.
